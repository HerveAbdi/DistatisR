...
#' (do nothing), \code{SUMPCA} (normalize by the total inertia) or \code{MFA} (\code{default} ) that normalizes each matrix so
#' that its first eigenvalue is equal to one.
#' @param Distance if \code{TRUE} (\code{default}) the matrices are distance matrices,
#' @param double_centering if \code{TRUE} (\code{default}) the matrices are double-centered,
...
distatis <- function(LeCube2Distance,
  ...
  if (compact == FALSE) {
    ...
    F <-
      t(apply(eigenSplus$vectors, 1, '*', t(t(
